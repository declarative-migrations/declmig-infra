...
variable "description" {
  type    = string
  default = "Declarative Migrations service, worker, and Lambda OCI images"
}
...
variable "tags" {
  ...
  default = {
    ManagedBy = "terraform"
    Purpose   = "oci-images"
    Product   = "declarative-migrations"
  }
}
...
variable "azure_resource_group_name" { type = string, default = "declmig-oci" }
...
variable "r2_bucket_name" { type = string, default = "declmig-oci-archive" }
...
check "provider_specific_inputs" {
  assert {
    condition     = !var.enable_gcp_artifact_registry || trimspace(coalesce(var.gcp_project_id, "")) != ""
  ...
  assert {
    condition = !var.enable_cloudflare_r2_archive || (
      trimspace(coalesce(var.cloudflare_account_id, "")) != "" && trimspace(var.r2_bucket_name) != ""
    )
    error_message = "cloudflare_account_id and r2_bucket_name are required when enable_cloudflare_r2_archive is true."
  }
}

output "registry_modules_commit" { value = local.registry_modules_commit }
output "aws_ecr_repository_url" { value = try(module.aws_ecr[0].repository_url, null) }
output "gcp_artifact_registry_repository" { value = try(module.gcp_artifact_registry[0].docker_repository, null) }
output "azure_container_registry_login_server" { value = try(module.azure_acr[0].login_server, null) }
output "cloudflare_r2_archive_bucket" { value = try(module.cloudflare_r2_archive[0].bucket_name, null) }
output "cloudflare_r2_direct_oci_registry" { value = false }
