...
fail() { printf 'error: %s\n' "$*" >&2; exit 70; }
[[ $# -eq 0 ]] || { printf 'error: configuration is environment-only; command arguments are not accepted\n' >&2; exit 64; }
...
if [[ -n "${OCI_PUBLISHER_FILE:-}" ]]; then
  ...
  curl --proto '=https' --tlsv1.2 --fail --silent --show-error --location "$upstream_url" --output "$publisher"
fi
actual_blob="$(git hash-object "$publisher")"
[[ "$actual_blob" == "$upstream_blob" ]] || fail "publisher integrity mismatch: expected ${upstream_blob}, got ${actual_blob}"
case "${OCI_TOOLKIT_VERIFY_ONLY:-false}" in
  true) printf 'verified OCI publisher: zed-pkg/zed-infra@%s blob=%s\n' "$upstream_commit" "$upstream_blob" ;;
  false) bash "$publisher" ;;
