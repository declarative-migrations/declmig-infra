# OCI registry adapter for `declarative-migrations/declmig-infra`
...
PROMOTE_OCI_DRY_RUN=1 ./scripts/promote-oci-image.sh \
  ...
  registry.example/declmig:sha-0123456789ab portable
```

Use OIDC/workload identity or credential helpers. Do not put PATs, access keys, service-account JSON, Docker passwords, R2 credentials, production database URLs, migration credentials, live schema snapshots, or customer data in this repository, Terraform state, image labels, or image layers. Review cost, region, cleanup, IAM, migration safety, and rollback before apply or Crossplane sync. Production deployment manifests must pin the emitted digest.
